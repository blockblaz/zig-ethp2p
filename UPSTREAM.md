...
`src/transport/eth_ec_quic.zig` mirrors **ALPN** `eth-ec-broadcast` and high-level **quic-go-style** limits from ethp2p `sim/host.go`. The QUIC stack is [zquic](https://github.com/ch4r10t33r/zquic) v1.2.1 (pure Zig, TLS 1.3), fetched as a Zig package dependency.
...
- `zquic_quic_shim.zig` classifies incoming streams by `stream_id % 4` (RFC 9000 §2.1: 2 = server-initiated UNI, 3 = client-initiated UNI)
- `incoming_uni_streams` queue holds peer-initiated UNI streams; `tryAcceptIncomingUniStream` pops them
- `streamMakeUni` opens an outgoing UNI stream via `rawAllocateNextLocalUniStream`
