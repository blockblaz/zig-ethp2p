...
| QUIC incoming-stream limits (`MaxIncomingStreams` / `MaxIncomingUniStreams`) plumbed `EthEcQuicConfig` → `toQuicConfig` → shim `QuicConfig` (no longer silently dropped); applying them to the advertised transport params needs a zquic `ServerConfig` field (upstream follow-up) | [`sim/host.go`](https://github.com/ethp2p/ethp2p/blob/main/sim/host.go) `defaultQuicConfig` | `transport` (`zquic_quic_shim.QuicConfig`, `toQuicConfig`) |
