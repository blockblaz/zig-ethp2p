...
| Handshake protocol-version negotiation (`validateProtocolVersion`: clamp `> v1` to `v1`, reject `!= v1` with `ProtocolMismatch`); enforced on the inbound BCAST handshake | [`broadcast/peer.go`](https://github.com/ethp2p/ethp2p/blob/main/broadcast/peer.go) `validateProtocolVersion` | `wire.bcast_stream`, `broadcast.engine_quic` |
...
| Parked-chunk buffering (chunks arriving before their session; per-message cap `maxParkedChunks = 32`, replay on attach) + poll-driven `cleanup` (`activeSessionTTL` / `pendingChunkTTL` / `cleanupInterval` / `channelInboxCap` / `routingTickInterval` constants) | [`broadcast/channel.go`](https://github.com/ethp2p/ethp2p/blob/main/broadcast/channel.go) `handleChunk` / `handleSessionOpen` / `cleanup` | `broadcast.channel_rs` (`parkChunk` / `cleanup`) |
...
| Scenario stats collector (`NodeEvent` / `BandwidthEvent` / `ChunkStats` / `ScenarioStats`; publish/receive correlation → per-node delivery + latency, transport-byte totals, chunk-verdict tallies) — synchronous `record*` methods in place of Go's channel/goroutine `Run` | [`sim/collector.go`](https://github.com/ethp2p/ethp2p/blob/main/sim/collector.go) | `sim.collector` (`StatsCollector`) |
...
| QUIC incoming-stream limits (`MaxIncomingStreams` / `MaxIncomingUniStreams`) plumbed `EthEcQuicConfig` → `toQuicConfig` → shim `QuicConfig` → zquic `io.ServerConfig.max_incoming_*` (v1.7.67+), advertised as `initial_max_streams_{bidi,uni}` so peers may open the configured number of per-chunk streams | [`sim/host.go`](https://github.com/ethp2p/ethp2p/blob/main/sim/host.go) `defaultQuicConfig` | `transport` (`zquic_quic_shim`, `toQuicConfig`) |
| QUIC transport control ops: per-connection `ConnectionStats` (bytes sent/received) and real stream `Reset` / `CancelRead` / `CancelWrite` (RESET_STREAM / STOP_SENDING via zquic v1.7.68+; `streamResetReceived` reads the peer's error code) | [`transport/quic/quic.go`](https://github.com/ethp2p/ethp2p/blob/main/transport/quic/quic.go) `ConnectionStats` / `Reset` / `CancelRead` / `CancelWrite` | `transport` (`PeerConn.connectionStats`, `zquic_quic_shim.streamCancel*` / `streamResetReceived`) |
...
- **CI:** aligned with [ethp2p's `ci.yml`](https://github.com/ethp2p/ethp2p/blob/main/.github/workflows/ci.yml): **`markdown-lint`** checks out [ethp2p](https://github.com/ethp2p/ethp2p) at the revision pinned in `UPSTREAM.md` and runs [rumdl](https://rumdl.dev/) on `specs/`; `zig build test-broadcast`, `test-sim-rs`, `test-sim-gossipsub` (Debug + TSan), `test-quic` (**`quic-transport`** job: **45m** job timeout + **`timeout 40m`** on the command so a hung poll loop cannot exhaust the runner), `test-stress-ci` on **`main` only**, plus lint (`zig fmt --check`, `zig build`, `zig ast-check`). `build.zig.zon` **`minimum_zig_version`** must match workflow **`ZIG_VERSION`**; `just check-zig-ci-align` checks that locally. Run **`just specs-lint`** to mirror **`markdown-lint`** locally (needs `git` and `rumdl`).
